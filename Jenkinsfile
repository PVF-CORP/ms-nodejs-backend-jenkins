pipeline {
    agent {
        docker { image 'devops-agent:latest' }
    }

    environment {
        APELLIDO = "vilcapoma" // Cambiar por apellido
        ACR_NAME = "acrglobalcicd"
        ACR_LOGIN_SERVER = "${ACR_NAME}.azurecr.io"
        IMAGE_NAME = "my-nodejs-app-${APELLIDO}"
        RESOURCE_GROUP = "rg-cicd-terraform-app-baraujo03"
        AKS_NAME = "aks-dev-eastus"
    }

    stages {

        stage('Hello world') {
            steps {
                script {
                    env.VARIABLE = "demo123"
                }
                sh '''
                  echo ">>> Impresión Hello world"
                  echo "Hello world"
                  echo "Variable declarada en script: $VARIABLE"
                  echo "Variable declarada en environment: $APELLIDO"
                '''
                sh '''
                  echo ">>> Versiones instaladas:"
                  node -v
                  npm -v
                  docker --version
                  az version
                '''
            }
        }

        stage('[CI] Install dependencies') {
            steps {
                sh '''
                  echo ">>> Instalando dependencias..."
                  npm install
                '''
            }
        }

        stage('[CI] Unit tests') {
            steps {
                sh '''
                  echo ">>> Ejecutando pruebas unitarias..."
                  npm run test:unit
                '''
            }
        }

        stage('[CI] Integration tests') {
            steps {
                sh '''
                  echo ">>> Ejecutando pruebas de integración..."
                  npm run test:integration
                '''
            }
        }

        stage('Azure Login') {
            steps {
                withCredentials([
                    string(credentialsId: 'azure-clientId',       variable: 'AZ_CLIENT_ID'),
                    string(credentialsId: 'azure-clientSecret',   variable: 'AZ_CLIENT_SECRET'),
                    string(credentialsId: 'azure-tenantId',       variable: 'AZ_TENANT_ID'),
                    string(credentialsId: 'azure-subscriptionId', variable: 'AZ_SUBSCRIPTION_ID')
                ]) {
                    sh '''
                      echo ">>> Azure login..."
                      az login --service-principal \
                        --username="$AZ_CLIENT_ID" \
                        --password="$AZ_CLIENT_SECRET" \
                        --tenant="$AZ_TENANT_ID"

                      az account set --subscription $AZ_SUBSCRIPTION_ID
                    '''
                }
            }
        }

        stage('AKS Credentials') {
            steps {
                sh '''
                  echo ">>> Obteniendo credenciales de AKS..."
                  az aks get-credentials \
                    --resource-group $RESOURCE_GROUP \
                    --name $AKS_NAME \
                    --overwrite-existing
                '''
            }
        }

        stage('[CI] Get Git Commit Short SHA') {
            steps {
                script {
                    env.IMAGE_TAG = sh(script: "git rev-parse --short HEAD", returnStdout: true).trim()
                    echo "IMAGE_TAG generado: ${env.IMAGE_TAG}"
                }
            }
        }

        stage('[CI] Build & Push to ACR') {
            steps {
                sh '''
                  echo ">>> Login al ACR..."
                  az acr login --name $ACR_NAME

                  echo ">>> Build de imagen..."
                  docker build -t $ACR_LOGIN_SERVER/$IMAGE_NAME:$IMAGE_TAG .

                  echo ">>> Push al ACR..."
                  docker push $ACR_LOGIN_SERVER/$IMAGE_NAME:$IMAGE_TAG
                '''
            }
        }

        // ============================ DEV ============================
        stage('[CD-DEV] Set Image Tag in k8s.yml') {
            steps {
                script {
                    env.API_PROVIDER_URL = "https://dev.api.com"
                    env.ENV = "dev"
                }
                sh '''
                  echo ">>> Renderizando k8s-dev.yml..."
                  envsubst < k8s.yml > k8s-dev.yml
                  cat k8s-dev.yml
                '''
            }
        }

        stage('[CD-DEV] Deploy to AKS') {
            steps {
                sh '''
                  az aks command invoke \
                    --resource-group $RESOURCE_GROUP \
                    --name $AKS_NAME \
                    --command "kubectl apply -f k8s-dev.yml" \
                    --file k8s-dev.yml
                '''
            }
        }

        stage('[CD-DEV] Get LoadBalancer IP') {
            steps {
                sh '''
                  echo ">>> Obteniendo IP del LoadBalancer (dev)..."
                  SERVICE_NAME="my-nodejs-service-${APELLIDO}-dev"
                  LB_IP=""
                  MAX_RETRIES=5
                  RETRY_COUNT=0

                  while [ -z "$LB_IP" ] && [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
                    LB_IP=$(az aks command invoke \
                      --resource-group $RESOURCE_GROUP \
                      --name $AKS_NAME \
                      --command "kubectl get svc $SERVICE_NAME -o jsonpath='{.status.loadBalancer.ingress[0].ip}'" \
                      --query 'logs' -o tsv | tr -d '[:space:]')
                    if [ -z "$LB_IP" ]; then
                      RETRY_COUNT=$((RETRY_COUNT+1))
                      echo "Intento $RETRY_COUNT/$MAX_RETRIES: IP aún no asignada, esperando 10s..."
                      sleep 10
                    fi
                  done

                  if [ -z "$LB_IP" ]; then
                    echo ">>> No se pudo obtener la IP del LoadBalancer (dev)."
                  else
                    echo ">>> [DEV] IP del LoadBalancer: $LB_IP"
                  fi
                '''
            }
        }

        // ====================== APROBACIÓN QA ========================
        stage('Aprobación QA') {
            steps {
                timeout(time: 10, unit: 'MINUTES') {
                    input message: '¿Aprobar despliegue a QA?', ok: 'Sí, desplegar a QA'
                }
            }
        }

        // ============================ QA =============================
        stage('[CD-QA] Set Image Tag in k8s.yml') {
            steps {
                script {
                    env.API_PROVIDER_URL = "https://qa.api.com"
                    env.ENV = "qa"
                }
                sh '''
                  echo ">>> Renderizando k8s-qa.yml..."
                  envsubst < k8s.yml > k8s-qa.yml
                  cat k8s-qa.yml
                '''
            }
        }

        stage('[CD-QA] Deploy to AKS') {
            steps {
                sh '''
                  az aks command invoke \
                    --resource-group $RESOURCE_GROUP \
                    --name $AKS_NAME \
                    --command "kubectl apply -f k8s-qa.yml" \
                    --file k8s-qa.yml
                '''
            }
        }

        stage('[CD-QA] Get LoadBalancer IP') {
            steps {
                sh '''
                  echo ">>> Obteniendo IP del LoadBalancer (qa)..."
                  SERVICE_NAME="my-nodejs-service-${APELLIDO}-qa"
                  LB_IP=""
                  MAX_RETRIES=5
                  RETRY_COUNT=0

                  while [ -z "$LB_IP" ] && [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
                    LB_IP=$(az aks command invoke \
                      --resource-group $RESOURCE_GROUP \
                      --name $AKS_NAME \
                      --command "kubectl get svc $SERVICE_NAME -o jsonpath='{.status.loadBalancer.ingress[0].ip}'" \
                      --query 'logs' -o tsv | tr -d '[:space:]')
                    if [ -z "$LB_IP" ]; then
                      RETRY_COUNT=$((RETRY_COUNT+1))
                      echo "Intento $RETRY_COUNT/$MAX_RETRIES: IP aún no asignada, esperando 10s..."
                      sleep 10
                    fi
                  done

                  if [ -z "$LB_IP" ]; then
                    echo ">>> No se pudo obtener la IP del LoadBalancer (qa)."
                  else
                    echo ">>> [QA] IP del LoadBalancer: $LB_IP"
                  fi
                '''
            }
        }

        // ===================== APROBACIÓN PRD ========================
        stage('Aprobación PRD') {
            steps {
                timeout(time: 10, unit: 'MINUTES') {
                    input message: '¿Aprobar despliegue a PRD?', ok: 'Sí, desplegar a PRD'
                }
            }
        }

        // ============================ PRD ============================
        stage('[CD-PRD] Set Image Tag in k8s.yml') {
            steps {
                script {
                    env.API_PROVIDER_URL = "https://api.com"
                    env.ENV = "prd"
                }
                sh '''
                  echo ">>> Renderizando k8s-prd.yml..."
                  envsubst < k8s.yml > k8s-prd.yml
                  cat k8s-prd.yml
                '''
            }
        }

        stage('[CD-PRD] Deploy to AKS') {
            steps {
                sh '''
                  az aks command invoke \
                    --resource-group $RESOURCE_GROUP \
                    --name $AKS_NAME \
                    --command "kubectl apply -f k8s-prd.yml" \
                    --file k8s-prd.yml
                '''
            }
        }

        stage('[CD-PRD] Get LoadBalancer IP') {
            steps {
                sh '''
                  echo ">>> Obteniendo IP del LoadBalancer (prd)..."
                  SERVICE_NAME="my-nodejs-service-${APELLIDO}-prd"
                  LB_IP=""
                  MAX_RETRIES=5
                  RETRY_COUNT=0

                  while [ -z "$LB_IP" ] && [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
                    LB_IP=$(az aks command invoke \
                      --resource-group $RESOURCE_GROUP \
                      --name $AKS_NAME \
                      --command "kubectl get svc $SERVICE_NAME -o jsonpath='{.status.loadBalancer.ingress[0].ip}'" \
                      --query 'logs' -o tsv | tr -d '[:space:]')
                    if [ -z "$LB_IP" ]; then
                      RETRY_COUNT=$((RETRY_COUNT+1))
                      echo "Intento $RETRY_COUNT/$MAX_RETRIES: IP aún no asignada, esperando 10s..."
                      sleep 10
                    fi
                  done

                  if [ -z "$LB_IP" ]; then
                    echo ">>> No se pudo obtener la IP del LoadBalancer (prd)."
                  else
                    echo ">>> [PRD] IP del LoadBalancer: $LB_IP"
                  fi
                '''
            }
        }
    }

    post {
        always   { echo "Pipeline finalizó (sin importar resultado)" }
        success  { echo "Pipeline CI/CD completado con éxito" }
        failure  { echo "Pipeline falló en algún stage" }
        aborted  { echo "Pipeline fue abortado manualmente" }
    }
}
